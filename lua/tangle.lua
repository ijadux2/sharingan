...
local function find_fenced_blocks_regex(bufnr)
  ...
  print("[tangle] total lines:", #lines)
  ...
  for i, line in ipairs(lines) do
    if not in_fence then
      ...
      if fence_match ~= nil then
        print("[tangle] found fence:", fence_match, "rest:", rest)
        in_fence = true
      ...
      if line:match("^%s*```%s*$") then
        ...
        print("[tangle] fence end, tangle_file:", tangle_file, "code_len:", #code)
  ...
  print("[tangle] found blocks:", #blocks)
  return blocks
end
...
handlers.markdown = {
  ...
  tangle_marker = function(info)
    local tangle_match = info:match("tangle%s*=%s*[\"'](.-)[\"']")
      or info:match("tangle:%s*(%S+)")
    return tangle_match
  end,
...
handlers.yaml = {
  ...
  tangle_marker = function(info)
    local key = info or ""
    local tangle_match = key:match("|([^|]+)$")
      or key:match("tangle%s*=%s*(%S+)")
      or key:match("tangle:%s*(%S+)")
    return tangle_match
  end,
  get_code = function(node, bufnr)
    local start_row, start_col, end_row, end_col = node:range()
...
function M.tangle()
  ...
  if not handler then
    vim.notify("[tangle] Unsupported filetype: " .. vim.bo[bufnr].filetype, vim.log.levels.WARN)
    return
  end
  ...
  if handler.parser then
    ...
    if not parser then
      vim.notify("[tangle] Parser not available for " .. vim.bo[bufnr].filetype .. ", using regex fallback", vim.log.levels.INFO)
